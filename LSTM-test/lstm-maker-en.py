...
train_vol = 0.95         # Сколько берем от объема для обучения
...
"""
model = tf.keras.Sequential()
model.add(tf.keras.layers.LSTM(1024, input_shape=(X_train.shape[1], X_train.shape[2]), return_sequences=True))
