...
def loadfileen(path, ticker, start_date, end_date, separator=","):
    print("Load data from Yahoo, ticker:", ticker)
    raw_data = pdr.get_data_yahoo(ticker, start_date, end_date)
    raw_data = raw_data[['Open', 'Low', 'High', 'Close', 'Adj Close', 'Volume']]
...
def loaddataen(path, ticker, separator):
    # Читаем файл
    main_ticker_data = pd.read_csv(path + ticker + '.csv', sep=separator)
    # Берем только нужные поля
    main_ticker_data = main_ticker_data[['Open', 'Low', 'High', 'Close', 'Adj Close', 'Volume']]
    # Переводим из формата pandas.DataFrame в np.array
    return main_ticker_data.values
